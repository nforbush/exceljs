...
// *** ADD ***
const VmlFmlaTxbxXform = require("./vml-fmla-txbx-xform");
const VmlFmlaLinkXform = require("./vml-fmla-link-xform");
// *** ADD END ***
...
class VmlClientDataXform extends BaseXform {
  ...
  parseOpen(node) {
    // *** ADD ***
    if (this.map["x:FmlaTxbx"] === undefined)
      this.map["x:FmlaTxbx"] = new VmlFmlaTxbxXform();
    if (this.map["x:FmlaLink"] === undefined)
      this.map["x:FmlaLink"] = new VmlFmlaLinkXform();
    // *** ADD END ***
    switch (node.name) {
      case this.tag:
        this.reset();
        this.model = {
          objectType: node.attributes.ObjectType || "", // *** ADD ***
          anchor: [],
          protection: {},
          editAs: '',
        };
  ...
  normalizeModel() {
    ...
    // *** ADD ***
    if (this.map['x:FmlaTxbx'].model)
      this.model.formulaTextbox = this.map['x:FmlaTxbx'].model.text || "";
    if (this.map['x:FmlaLink'].model)
      this.model.formulaLink = this.map['x:FmlaLink'].model.text || "";
    // *** ADD END ***
  }
}
