...
// *** ADD ***
const AlternateContentXform = require('./alternate-content-xform');
const ChartObject = require('../../../doc/chart-object');
const Shape = require('../../../doc/shape');
const Control = require('../../../doc/control');
// *** ADD END ***
...
class WorkSheetXform extends BaseXform {
  ...
  parseOpen(node) {
    ...
    // *** ADD ***
    if (!this.map['mc:AlternateContent'])
      this.map['mc:AlternateContent'] = new AlternateContentXform();
    // *** ADD END ***
    if (this.map[node.name] && !this.ignoreNodes.includes(node.name)) { // *** MODIFY
    ...
    switch (name) {
      case 'worksheet': {
        ...
        this.model = {
          ...
          alternateContent: this.map['mc:AlternateContent'].model,    // *** ADD ***
        };
  ...
  reconcile(model, options) {
    ...
    if (model.drawing) {
      ...
      if (match) {
        ...
        drawing.anchors.forEach(anchor => {
          ...
          // *** ADD ***
          if (anchor.graphicFrame && anchor.graphicFrame.graphic.graphicData.chart)
            model.charts.push(new ChartObject(anchor, model.name, options));
          model.shapes.push(new Shape(anchor));
          // *** ADD END ***
        });
      }
    }
    ...
    // *** ADD ***
    model.controls = [];
    if (model.alternateContent && model.alternateContent.controls) {
      const indexes = {CheckBox: 0, Radio: 0, Button: 0, GBox: 0};
      model.alternateContent.controls.forEach(control => {
        const ctrlPropRel = rels[control.attributes.rid];
        const ctrlProp = options.ctrlProps[ctrlPropRel.Target];
        let shape = null;
        Object.entries(options.vmlDrawings).every(([key, value]) => {
          const ctrl = value.controls.filter(ctr => ctr.id === `_x0000_s${control.attributes.shapeId}` || ctr.id === control.attributes.name)[0];
          if (ctrl) {
            shape = ctrl;
            return false;
          }
        ...
        const shapeMatch = model.shapes.filter(shp => shp.id === control.attributes.shapeId)[0];
        if (shapeMatch) {
          shapeMatch.control = shapeMatch;
          const controlObj = new Control(ctrlProp, control, shape, shapeMatch, indexes[ctrlProp.objectType]++);
          model.controls.push(controlObj);
        }
      });
      model.controls.forEach(control => {
        if (control.objectType === 'Radio') {
          const groupBox = model.controls.filter(cntrl => cntrl.objectType === 'GBox' && control.range.intersects(cntrl.range))[0];
          if (groupBox) {
            control.groupBox = groupBox;
            if (control.formulaLink)
              groupBox.formulaLink = control.formulaLink;
          }
        }
      });
    }
    ...
    delete model.alternateContent; // *** ADD ***
  }
}
