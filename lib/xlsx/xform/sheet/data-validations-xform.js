  ...
  parseClose(name) {
    switch (name) {
      ...
      case 'dataValidation': {
          ...
          if (addr.includes(':')) {
            ...
            range.forEachAddress(address => {
              if (this.model[address] === undefined) {              // *** ADD ***
                let dv = formulaeRanges ? adjustFormulaeRanges(this._dataValidation, address, rangeFirst) : null;   // *** ADD ***
                this.model[address] = dv || this._dataValidation;   // *** MODIFY ***
              }                                                     // *** ADD ***
            });
          } else {
            if (this.model[addr.address] === undefined) {             // *** ADD ***
              let dv = formulaeRanges ? adjustFormulaeRanges(this._dataValidation, addr.address, rangeFirst) : null;   // *** ADD ***
        ...
        return true;
        // *** ADD ***
        function adjustFormulaeRanges(_dataValidation, address, rangeFirst) {
          if (!formulaeRanges)
            return null;
          let dv = null;
